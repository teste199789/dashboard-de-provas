...
const InfoTab = ({ proof, refreshProof }) => {
    ...
    useEffect(() => {
        // Inicializa o estado com as matérias existentes ou uma linha em branco
        setSubjects(proof.subjects && proof.subjects.length > 0 ? proof.subjects.map(s => ({...s})) : [{ id: 1, nome: '', questoes: '' }]);
    }, [proof.subjects]);

    const handleAddSubject = () => {
        setSubjects([...subjects, { id: Date.now(), nome: '', questoes: '' }]);
    };
    ...
    const handleRemoveSubject = (index) => {
        if (subjects.length > 0) { // Permite remover até a última linha
    ...
    const handleSaveChanges = async () => {
        ...
        try {
            // Envia apenas as matérias e o total de questões recalculado
    ...
    return (
        <div className="p-6">
            ...
            <p className="text-sm text-gray-500 dark:text-gray-400 mb-4">Defina as matérias e a quantidade de questões de cada uma. O total de questões da prova será atualizado automaticamente.</p>
            <div className="space-y-3">
                {subjects.map((subject, index) => (
                    <div key={subject.id || index} className="flex items-center gap-2 p-2 rounded-md hover:bg-gray-100 dark:hover:bg-gray-700">
                        ...
                        <button onClick={() => handleRemoveSubject(index)} className="text-red-500 p-2 hover:bg-red-100 dark:hover:bg-red-900/50 rounded-full"><TrashIcon className="w-5 h-5"/></button>
                    </div>
                ))}
            </div>
